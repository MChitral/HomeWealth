...
export function calculateMonthlyPayment(
  ...
  const payment =
    (periodicRate * principal) / (1 - Math.pow(1 + periodicRate, -amortizationMonths));
 ...
 */
export function calculatePrincipalPayment(paymentAmount: number, interestAmount: number): number {
  // Round to nearest cent (Canadian lender convention)
...
export function calculateRemainingAmortization(
  ...
  const remainingPayments =
    -Math.log(1 - (periodicRate * remainingBalance) / paymentAmount) / Math.log(1 + periodicRate);
...
export function generateAmortizationSchedule(
  ...
  while (remainingBalance > 0.01 && paymentNumber <= maxPayments) {
    ...
    for (const prep of eligibleAnnualPrepayments) {
      ...
      const yearsSinceStart = Math.floor(
        (paymentNumber - prep.startPaymentNumber) / paymentsPerYear
      );
      const expectedPaymentForThisYear =
        prep.startPaymentNumber + yearsSinceStart * paymentsPerYear;
  ...
  const payoffDate =
    lastPayment && lastPayment.remainingBalance < 0.01 ? lastPayment.paymentDate : null;
  ...
  const averageAmortizationMonths =
    validPaymentCount > 0 ? totalRemainingAmortizationMonths / validPaymentCount : 0;
