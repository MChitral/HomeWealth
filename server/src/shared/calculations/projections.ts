...
function generatePrepayments(
  ...
  if (prepaymentPercent > 0 && monthlySurplus > 0 && basePaymentAmount > 0) {
    ...
    const perPaymentExtra = paymentsPerMonth > 0 ? monthlyPrepayAmount / paymentsPerMonth : 0;
    const percentOfPayment = perPaymentExtra > 0 ? (perPaymentExtra / basePaymentAmount) * 100 : 0;
...
export function generateProjections(
  ...
  for (let year = 1; year <= maxYears; year++) {
    ...
    projections.push({
      ...
      cumulativePrincipal: Math.round(mortgageData.cumulativePrincipal),
    });
  }
...
export function calculateScenarioMetrics(
  ...
  const efStatus =
    efCurrent >= efTarget ? "Funded" : `${Math.round((efCurrent / efTarget) * 100)}% funded`;
