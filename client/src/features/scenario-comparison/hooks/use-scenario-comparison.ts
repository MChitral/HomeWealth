...
import type { ScenarioWithProjections, ScenarioForSelection, TimeHorizon, MetricName } from "../types";
...
export function useScenarioComparison() {
  ...
  const scenariosMap = useMemo(() => {
    return (scenariosWithMetrics || []).reduce((acc: Record<string, ScenarioWithProjections>, scenario, index) => {
      acc[scenario.id] = {
        ...scenario,
