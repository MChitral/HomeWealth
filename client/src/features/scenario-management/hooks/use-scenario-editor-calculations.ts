...
export function useScenarioEditorCalculations({
  ...
  const currentMortgageData = useMemo(() => {
    ...
    if ((!paymentAmount || paymentAmount <= 0) && currentBalance > 0 && amortizationMonths > 0 && currentRate > 0) {
      paymentAmount = calculatePayment(currentBalance, currentRate / 100, amortizationMonths, paymentFrequency);
    }
  ...
  }, [mortgage, latestTerm, latestPayment, totalPrincipalPaid, totalInterestPaid, yearsIntoMortgage, paymentFrequency]);
  ...
  }, [currentMortgageData.currentBalance, currentMortgageData.monthlyPayment, rateUsedForPreview, paymentFrequency]);
