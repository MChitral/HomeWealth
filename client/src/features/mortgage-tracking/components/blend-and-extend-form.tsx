...
import {
  useBlendAndExtendForm,
  type BlendAndExtendFormData,
} from "../hooks/use-blend-and-extend-form";
...
function BlendAndExtendFormFields({
  ...
  useEffect(() => {
    if (marketRate?.rate) {
      setValue("newMarketRate", marketRate.rate.toFixed(2));
    }
  }, [marketRate, setValue]);
  ...
  const amortizationOptions = [
    {
      value: remainingAmortizationMonths.toString(),
      label: `${Math.round(remainingAmortizationMonths / 12)} years (current)`,
    },
    {
      value: originalAmortizationMonths.toString(),
      label: `${Math.round(originalAmortizationMonths / 12)} years (original)`,
    },
  ];
    ...
    <div className="space-y-4 py-4">
      <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <p className="text-sm text-blue-900 dark:text-blue-100">
          <strong>Blend-and-Extend:</strong> Combine your current rate with the new market rate, and
          optionally extend your amortization period to lower your payments.
        </p>
      </div>
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormDescription>
              Extend your amortization to lower payments. Defaults to original amortization if not
              specified.
            </FormDescription>
            <FormMessage />
          </FormItem>
